...
export default function OBSNavigator({ max, title }) {
    ...
    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <Box sx={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
        <ButtonGroup>
          <IconButton
            disabled={obs[0] <= 1}
            onClick={() => setObs([obs[0] - 1 > 0 ? obs[0] - 1 : obs[0], 0])}
          >
            <KeyboardDoubleArrowLeftIcon fontSize="medium" />
          </IconButton>
          <IconButton
            disabled={obs[1] <= 0}
            onClick={() => setObs([obs[0], obs[1] - 1 >= 0 ? obs[1] - 1 : obs[1]])}
          >
        ...
        <ButtonGroup>
          <IconButton
            disabled={obs[1] >= max}
            onClick={() => setObs([obs[0], obs[1] + 1 <= max ? obs[1] + 1 : obs[1]])}
          >
            <KeyboardArrowRightIcon fontSize="medium" />
          </IconButton>
          <IconButton
            disabled={obs[0] >= 50}
            onClick={() => setObs([obs[0] + 1 <= 50 ? obs[0] + 1 : obs[0], 0])}
          >
